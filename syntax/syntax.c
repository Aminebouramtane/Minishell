...
/*   Updated: 2024/09/06 10:02:12 by abouramt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_echo_command(t_datatoken **node)
{
	...
	{
		*node = (*node)->next;
		while (*node && (*node)->cmd[0] == ' ' && (*node)->e_state == 2)
			*node = (*node)->next;
		if (!(*node))
			return (0);
...
{
	if (node && node->cmd[0] == '|' && node->e_state == 2 && len == 1)
	{
...
{
	if (*node && (*node)->next && (*node)->cmd[0] == '|'
		&& (*node)->e_state == 2)
	{
		*node = (*node)->next;
		while ((*node)->cmd[0] == ' ' && (*node)->e_state == 2)
			*node = (*node)->next;
		if (!(*node))
		{
			ft_putstr_fd("syntax error near unexpected token `|'\n", 2);
		...
		if ((*node)->cmd[0] == '|' && (*node)->e_state == 2)
		{
			ft_putstr_fd("syntax error near unexpected token `||'\n", 2);
			return (1);
		}
		if ((*node)->cmd[0] == '>' && (*node)->e_state == 2 && !(*node)->next)
		{
			ft_putstr_fd("syntax error near unexpected token `|'\n", 2);
			return (1);
		}
...
{
	if (node && !node->next && ((node->cmd[0] == '>' && node->e_state == 2)
			...
			|| (node->cmd[0] == '<' && node->e_state == 2)))
	{
		...
		if (!node)
			return (0);
		if ((node->cmd[0] == '|' || node->cmd[0] == '>') && node->e_state == 2)
		{
			ft_putstr_fd("syntax error near unexpected token `|'\n", 2);
			return (1);
		}
